...
public:
   ...
   */
  explicit Oauthtwo(const Json::Value& pJson) noexcept(false);
  ...
  /// Get the value of the column provider_id, returns the default value if the
  /// column is null
  const int64_t&
  getValueOfProviderId() const noexcept;
  /// Return a shared_ptr object pointing to the column const value, or an empty
  /// shared_ptr object if the column is null
  const std::shared_ptr<int64_t>&
  getProviderId() const noexcept;
  /// Set the value of the column provider_id
  void
  setProviderId(const int64_t& pProviderId) noexcept;
  void
  setProviderIdToNull() noexcept;
...
private:
  friend drogon::orm::Mapper<Oauthtwo>;
#ifdef __cpp_impl_coroutine
  friend drogon::orm::CoroMapper<Oauthtwo>;
#endif
  ...
  std::shared_ptr<int64_t> providerId_;
