...
public:
   ...
   */
  explicit GrantedPermission(const Json::Value& pJson) noexcept(false);
  ...
  /// Get the value of the column permission_id, returns the default value if
  /// the column is null
  const int64_t&
  getValueOfPermissionId() const noexcept;
  /// Return a shared_ptr object pointing to the column const value, or an empty
  /// shared_ptr object if the column is null
  const std::shared_ptr<int64_t>&
  getPermissionId() const noexcept;
  /// Set the value of the column permission_id
  void
  setPermissionId(const int64_t& pPermissionId) noexcept;
  void
  setPermissionIdToNull() noexcept;
...
private:
  friend drogon::orm::Mapper<GrantedPermission>;
#ifdef __cpp_impl_coroutine
  friend drogon::orm::CoroMapper<GrantedPermission>;
#endif
  ...
  std::shared_ptr<int64_t> permissionId_;
